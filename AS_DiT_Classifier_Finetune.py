...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (index, ecg, x, y) in linear_train_bar:
            ...
            logits = classifier(x, t, ecg)
            loss = criteon(logits, y)
            classifier_opt.zero_grad()
            loss.backward()
            classifier_opt.step()
            linear_train_bar.set_description('Finetune train')


        classifier.eval()
        linear_test_bar = tqdm(enumerate(testLoader), total=len(testLoader), leave=True, ncols=100, position=0)
        linear_train_bar.set_description('Finetune Test')
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classifier_lr', default=1e-4, type=float, help='分类头学习率')
    parser.add_argument('--experiment_name', default='AS-DiT-加入对比学习loss-dim256-Finetune', type=str, help='本次实验名称')
    ...
    parser.add_argument('--flag', default=True, type=bool, help='是否微调')
